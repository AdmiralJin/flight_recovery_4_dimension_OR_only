...
function updateSummary() {
  ...
  const readinessText = readiness?.solve_ready ? "READY" : `NOT READY${readiness ? `: ${(readiness.missing_inputs || []).map(missingInputLabel).join(", ")}` : ""}`;
...
function currentSolveBundle() {
  ...
  return {
    ...
    recovery_columns: clone(workbenchState.recoveryColumns),
    capacity_profile: clone(workbenchState.passengerCapacityProfile),
...
function updateSolveButton() {
  ...
  if (workbenchState.solving) status.textContent = `Solving exact recovery · ${elapsedLabel()}${Date.now() - workbenchState.solveStartedAt > 30000 ? " · taking longer than the benchmark baseline" : ""}`;
    ...
    status.textContent = `Solve unavailable: ${missing.map(missingInputLabel).join(", ")}`;
...
async function runSolve() {
  ...
  try {
    const readiness = await checkSolveReadiness(currentSolveBundle());
    workbenchState.solveReadiness = readiness;
    if (!readiness.solve_ready) {
      showGlobalNotice(`Solve unavailable: ${(readiness.missing_inputs || []).map(missingInputLabel).join(", ")}`, "warning");
      return openRecovery();
    }
...
function resetToBaseline() {
  const baseline = workbenchState.baseline;
  if (!baseline) return;
  workbenchState.caseId = baseline.caseId;
  workbenchState.source = baseline.source;
  workbenchState.scenario = clone(baseline.scenario);
  workbenchState.solveBundle = clone(baseline.solveBundle);
  workbenchState.recoveryColumns = clone(baseline.recoveryColumns);
  workbenchState.passengerCapacityProfile = clone(baseline.passengerCapacityProfile);
  workbenchState.costOverrides = clone(baseline.costOverrides);
  workbenchState.costEffective = buildEffectiveCostProfile(workbenchState.costBaseline, workbenchState.costOverrides);
  workbenchState.scenarioValidation = null;
  workbenchState.constraintPrecheck = null;
  workbenchState.revision += 1;
...
async function initializeWorkbench() {
  try {
    const [costs, registry, examples] = await Promise.all([
      loadCanonicalCosts(),
      loadConstraintRegistry(),
      loadSolveExamples(),
    ]);
...
fileInput.addEventListener("change", async () => {
  ...
  try {
    ...
    const scenario = isSolveBundle ? candidate.scenario : imported;
    ...
    if (isSolveBundle) {
      ...
      applyScenario(result.normalized_data, result.normalized_data.scenario_id, "import");
    }
    await refreshSolveReadiness();
