...
Workbench v1.1 的 Example selector 由后端目录驱动，当前提供 `phase1_benchmark_001`、`toy_case_016_benders_branch_and_price` 和 scenario-only 的 `toy_case_001`。Solve-ready case 会作为完整 Solve Bundle 加载；scenario-only case 可以校验和查看，但会明确显示缺少的 Solve 输入。
...
`Export Scenario` 仅导出 Scenario；`Export Snapshot` 导出可再导入的 Scenario / Solve Bundle / cost overrides 工作台快照。
